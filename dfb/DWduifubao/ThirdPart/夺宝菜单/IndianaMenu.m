...
@property(nonatomic,strong)NSString *ClassOneBtntitle;
...
- (instancetype)initWithFrame:(CGRect)frame IsClass:(BOOL)iSClass ClassArr:(NSMutableArray*)arr ClassOneBtntitle:(NSString *)title{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row==0) {
        if (ISClass ==YES) {
            ...
            [cell.OneBtn setTitle:_ClassOneBtntitle forState:(UIControlStateNormal)];
            [cell.OneBtn setImagePosition:LXMImagePositionRight spacing:0.f];
        ...
        cell.IMenuTabTwoCellBlock =^(NSInteger tag){
           // [weakSelf reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:0]] withRowAnimation:(UITableViewRowAnimationNone)];
            ...
            [cell.OneBtn setTitle:((IndianaShopClassModel*)ClassArr[tag]).category_name forState:(UIControlStateNormal)];
            [cell.OneBtn setImagePosition:LXMImagePositionRight spacing:0.f];
            [cell.OneBtn setTitleColor:[UIColor grayColor] forState:(UIControlStateNormal)];
            [cell.TwoBtn setTitleColor:[UIColor redColor] forState:(UIControlStateNormal)];
            cell.IsOpen =@"0";
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row ==0) {
        return IMenuWidth*0.11;
    }else{
        return self.bounds.size.height-IMenuWidth*0.11;
...
-(void)TableViewIsOpen:(BOOL)Y{
    [UIView animateWithDuration:0.3 animations:^{
        self.frame = CGRectMake(x, y, [UIScreen mainScreen].bounds.size.width,Y==YES ? [UIScreen mainScreen].bounds.size.height:IMenuWidth*0.11);
        [self layoutIfNeeded];
    }];

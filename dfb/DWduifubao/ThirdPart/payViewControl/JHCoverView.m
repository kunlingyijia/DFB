...
- (void)textChange:(UITextField *)textField
{
    ...
    if (self.pwStr.length == 6) {
       // if ([self.pwStr isEqualToString:self.Password]) {//如果输入的密码是199103表示输入密码正确
            
           // NSLog(@"密码正确==%@",self.pwStr);
            
            if ([self.delegate respondsToSelector:@selector(inputCorrectCoverView: password:)]) {
                [self.delegate inputCorrectCoverView:self password:self.pwStr];
                [self deleteClick:nil];
